...
const readFunc = async (req, res) => {
  try {
    ...
    if (req.query.page && req.query.limit) {
      ...
      data = await db.Product.findAll({ attributes: ["id", "title", "product_id", "capacity_id", "color_id", "stock", "discount", "price", "desc", "slug", "category_id", "updatedAt", "createdAt"], order: [["title", "ASC"]] })
    }
...
const createFunc = async (req, res) => {
  try {
    ...
    let data = await db.Product.create({ title: title, product_id: product_id, capacity_id: capacity_id, color_id: color_id, stock: stock, discount: discount, price: price, desc: desc, category_id: category_id, slug: slug });
...
const updateFunc = async (req, res) => {
  try {
    ...
    await product.update({ title, product_id, capacity_id, color_id, stock, discount, price, desc, category_id, slug });
