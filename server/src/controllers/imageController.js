...
const readFunc = async (req, res) => {
  try {
    let data
    let { page, limit, config_id } = req.query;
    ...
    if (page && limit) {
      ...
      data = await db.Image.findAll({ attributes: ["id", "url", "file_name", "config_id", "updatedAt", "createdAt"], order: [["id", "ASC"]] });
    }
...
//   try {
//     const { url, file_name, config_id } = req.body.data;
//     if (!url || !file_name || !config_id) return res.status(200).json({ message: "missing required parameters", code: 1 });
//     let data = await db.Image.create({ url: url, file_name: file_name, config_id: config_id });
  ...
  try {
    ...
    const createdImages = await Promise.all(
      images.map(async (image) => {
        const { url, file_name, config_id } = image;

        if (!url || !file_name || !config_id) {
          throw new Error("missing required parameters");
        }

        return await db.Image.create({ url, file_name, config_id });
      })
    );
...
const updateFunc = async (req, res) => {
  try {
    ...
    if (image) {
      await image.update({ url: data.url, file_name: data.file_name, config_id: data.config_id });
