  ...
  try {
    ...
    if (req.query.page && req.query.limit) {
      ...
      let { count, rows } = await db.Position.findAndCountAll({
        offset: offset,
        limit: limit,
        attributes: ["id", "key_position", "name", "desc", "state", "is_master","updatedAt","createdAt"],
        order: [["key_position", "ASC"]],
      })
      const totalPages = Math.ceil(count / limit);
      data = { totalRows: count, totalPages: totalPages, positions: rows, }
    } else {
      data = await db.Position.findAll({ attributes: ["id", "key_position", "name", "desc", "state", "is_master","updatedAt","createdAt"], order: [["key_position", "ASC"]] })
    }
...
const createFunc = async (req, res) => {
  try {
    ...
    let data = await db.Position.create({ key_position: key_position, name: name, desc: desc, state: true, is_master: false });
...
const updateFunc = async (req, res) => {
  try {
    ...
    if (position) {
      await position.update({ key_position: data.key_position, name: data.name, desc: data.desc, state: data.state, is_master: data.is_master });
